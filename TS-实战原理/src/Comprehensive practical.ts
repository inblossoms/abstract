...
type KebabCaseToCamelCase<Str extends string> = Str extends `${infer first}-${infer res}` ? `${first}${KebabCaseToCamelCase<Capitalize<res>>}` : Str

// CamelCaseToKebabCase
type CamelCaseToKebabCaseRes = CamelCaseToKebabCase<'zhangSan'>

type CamelCaseToKebabCase<Str extends string> = Str extends `${infer first}${infer res}`
  ? first extends Lowercase<first>
    ? `${first}${CamelCaseToKebabCase<res>}`
    : `-${Lowercase<first>}${CamelCaseToKebabCase<res>}`
  : Str

// Chunk
// 对数组做分组，比如 1、2、3、4、5 的数组，每两个为 1 组，那就可以分为 1、2 和 3、4 以及 5 这三个 Chunk
type Chunk<Ary extends unknown[], aryItem extends number, CurItem extends unknown[] = [], Res extends unknown[] = []> = Ary extends [infer first, ...infer args]
...
// TupleToNestedObject
type TupleToNestedObject<Tuple extends unknown[], Value> = Tuple extends [infer first, ...infer args]
  ? {
      [key in first as key extends keyof any ? key : never]: args extends unknown[] ? TupleToNestedObject<args, Value> : Value
    }
  : Value
