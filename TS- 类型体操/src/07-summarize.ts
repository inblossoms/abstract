...
// 提取函数类型的返回值
type getFunToType<fn extends Function> = fn extends () => infer RetType
  ? RetType
  : never;
...
// 将长度不确定的字符串类型转换成联合类型
type StrToUnion<Str extends string> = Str extends `${infer first}${infer args}`
...
> = RetAry["length"] extends Length
  ? RetAry
  : BuildAry<Length, e, [...RetAry, e]>;

type Subtract1<x extends number, y extends number> = BuildAry<x> extends [
  ...X: BuildAry<y>,
  ...Y: infer result
]
  ? result["length"]
...
type union1 = "a" | "b" | "c";
type UppercaseC<i extends string> = i extends "c" ? Uppercase<i> : i;
...
> = {
  readonly [key in keyof obj | keyof retObj]: key extends keyof obj
    ? key extends keyof retObj
      ? MergeValues<obj[key], retObj[key]>
      : obj[key]
    : key extends keyof retObj
    ? retObj[key]
    : never;
};
type mergerParams = MergerParams<{ a: 1 }, { b: 2 }>;

type MergeValues<o, ret> = o extends ret
  ? o
  : ret extends unknown[]
  ? [o, ...ret]
  : [o, ret];
