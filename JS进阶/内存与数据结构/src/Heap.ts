...
class BinaryHeap {
  ...
  heap: any
  size: any
  constructor(compare: any, array?: any) {
  ...
  heapify(i: number) {
    ...
    const prv = this.heap[i],
      lv = this.heap[i],
      rv = this.heap[i]

    let small = i
    if (l < this.size && this.compare(lv, prv)) small = l
    if (r < this.size && this.compare(rv, this.heap[small])) small = r
    if (small != i) {
      this.swap(i, small)
      this.heapify(small)
    }
...
// 这时候针对这样的数组构架一个二叉堆，比较函数就会按照需求比较id，而非节点本身
type obj = {
  a: {}
  b: {}
}

function _compare<a extends Record<string, any>, b extends Record<string, any>>(
  a: { id: number },
  b: { id: number }
) {
  return a.id > b.id
